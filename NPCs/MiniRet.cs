...
namespace GLHF.NPCs
{
    public class MiniRet : ModNPC
    {
        int shootTimer;
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Mini Retinazer");     //The English name of the npc
        }
        ...
        public override void AI()
        {
            ...
            if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
            {
                ...
                if (shootTimer >= Main.rand.Next(60, 180))
                {
                    shootTimer = 0;
                    Projectile.NewProjectile(vector8.X, vector8.Y, (float)((Math.Cos(rotation) * Speed) * -1), (float)((Math.Sin(rotation) * Speed) * -1), ProjectileID.DeathLaser, 20, 0f, 0);
                }
            }
